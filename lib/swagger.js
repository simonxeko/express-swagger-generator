...
function parseTypedef(tags) {
    ...
    if (tags[0].type && tags[0].type.name) {
        details.allOf = tags[0].type.name.split("&").map((n) => {
        ...
        if (tags[i].title == 'property') {
            ...
            if (schema) {
                ...
                if (prop.format == 'objectId') {
                    example = Math.round(new Date() / 1000).toString(16) + 
                              Math.round(0xFFFFFFFFFF*Math.random()).toString(16).padStart(10,"0") +
                              Math.round(0xFFFFFF*Math.random()).toString(16).padStart(6,"0");
                }
                if (example) {
                    switch (type) {
                        case 'boolean':
    ...
    for (let i in comments) {
        ...
        if (i == 'tags') {
            ...
            for (let j in block) {
                ...
                if (title == 'output' && route) { //doctrine parser only parse "input" instead of input_example    
                    let example = parseExample(line);
                    // console.log("output example!!!", example);
                    // console.log("Found input example ", example);
                    for (let k in block) {
                        const line = block[k];
                        // console.log("iterate line title=", line.title, line);
                        if ((line.title == "return" || line.title == "returns") && line.description && line.description.startsWith(example.key)) {
                            // console.log("Found matching input");
                            line.examples = line.examples || {};
                            line.examples[example.displayName] = {
                                value: example.value
